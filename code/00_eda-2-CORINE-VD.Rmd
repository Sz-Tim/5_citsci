...
lc_VD %>% st_set_geometry(NULL) %>%
  ...
  theme(axis.text.x=element_text(angle=270, hjust=0, vjust=0.5, lineheight=0.7)) + 
...
lc_VD %>% st_set_geometry(NULL) %>%
  ...
  mutate(in_collections=cat_full %in% full_obs_props$cat_full,
         prop_collections=full_obs_props$prop_area[match(cat_full,
                                                         full_obs_props$cat_full)],
         code_full=paste0(str_sub(Dataset, 1, 2), "_", str_sub(cat_full, 1, 3))) %>%
